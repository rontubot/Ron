...
def _process_user_input(user_input, save_to_memory=True, username=None, task_manager=None):  
    ...
    try:
        ...
        if prof.get("enabled", True) and os.getenv("RON_PROFILE_TURN_CLASSIFIER", "1") == "1": 
            ...
            try:
                model_name = "gpt-5.4"
                cls = run_turn_classifier(_get_openai_client(), "gpt-5.1-chat-latest", original_input, snapshot)
            ...
            if do_batch:
                try:
                    model_name = "gpt-5.4"
                    batch = run_batch_profiler(_get_openai_client(), "gpt-5.1-chat-latest", prof["recent_window"])
