...
def detect_command_intent(user_input):  
    ...
    farewell_patterns = [  
        r"(vale|bueno|ok)?\\s*(hasta luego|adiós|chao|nos vemos|me voy)",  
        r"(vale|bueno|ok)?\\s*(desconéctate|apágate|hasta la vista)"  
    ]  
...
- "vale hasta luego" = comando de despedida  
...
- Despedida: 'hasta luego', 'adiós', 'desconéctate'  
...
def responder_a_usuario(user_input):  
    ...
    if command_intent:  
        if command_intent["type"] == "farewell":  
            ...
            return search_youtube(f"música de {query}")  
