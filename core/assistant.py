...
# NUEVA FUNCIÓN SIN MEMORIA AUTOMÁTICA PARA USUARIOS WEB  
def generate_response_no_memory(user_input):  
    """Versión de responder_a_usuario que NO guarda automáticamente en memoria"""  
    original_input = user_input  
    user_input = user_input.lower().strip()  
    ...
    # COMANDOS DIRECTOS EXISTENTES (sin memoria automática)  
    if user_input.startswith("abre "):  
        app_name = user_input.replace("abre ", "").strip()  
        result = open_application(app_name)  
        # NO llamar add_to_memory aquí  
    ...
    # NO guardar la conversación en memoria aquí  
    return ron_response  
